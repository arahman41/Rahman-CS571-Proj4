...
public class Desugar implements Expr.Visitor<Expr>, Stmt.Visitor<Stmt> {
    ...
    public Stmt visitForStmt(For stmt) {
        ...
        // Increment if exists
        if (stmt.incr != null) {
            List<Stmt> bodyWithIncr = new ArrayList<>();
            bodyWithIncr.add(stmt.body);
            bodyWithIncr.add(new Stmt.Expression(stmt.incr));
            whileBody = new Stmt.Block(bodyWithIncr);
        }

        Stmt whileLoop = new Stmt.While(condition, whileBody);
